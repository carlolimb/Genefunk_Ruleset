...
function getWoundPercent(v)
	...
	if sNodeType == "pc" then
		nHP = math.max(DB.getValue(nodeActor, "hp.total", 0), 0);
		nWounds = math.max(DB.getValue(nodeActor, "hp.wounds", 0), 0);
		nDeathSaveFail = DB.getValue(nodeActor, "hp.deathsavefail", 0);
	elseif sNodeType == "ct" then
		nHP = math.max(DB.getValue(nodeActor, "hptotal", 0), 0);
		nWounds = math.max(DB.getValue(nodeActor, "wounds", 0), 0);
		nDeathSaveFail = DB.getValue(nodeActor, "deathsavefail", 0);
	end
