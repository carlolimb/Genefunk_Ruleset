...
function addItems(sTypeGroup, sFilter)
	...
	for _,vMapping in ipairs(aMappings) do
		for _,vItem in pairs(DB.getChildrenGlobal(vMapping)) do	
			...
			if not StringManager.contains(aItemCheck, sItemLower) and string.match(sItemLower, string.lower(sFilter)) then
				if not string.match(sItemProperties, "magic") then
					table.insert(aItemCheck, sItemLower);

					local w;
					if sTypeGroup == "Armor" then
						w = createWindowWithClass("armorlist_item");
					elseif sTypeGroup == "Weapon" then
						w = createWindowWithClass("weaponlist_item");						
					else						
						w = createWindow();
					end
					local sItemCapitalized = StringManager.capitalizeAll(sItemLower);
					w.name.setValue(sItemCapitalized);
